...
def dashboard_registration(traces_path: typing.Type[Path],
    ...
    all_signals_norm = normalize_df(traces)
    ...
    source6 = ColumnDataSource(data=contour_dict)
    ...
    plot = figure(title='normalized  ' + exp_name + '_pos_' + signals_df.columns.values[0],
    ...
    tod_renderer = plot.line('tf', 'v_line', source=source4, line_color='red', line_width=4, line_dash='solid',
    ...
    def update_tf(attr, old, new):
        ...
        if image_path != source10.data['path'][0]:
            image_path = Path(bf_folder_path + selected_image[:-2] + '.tif')
...
def visualize_traces_wpoints(traces, registered_peaks, exp_name):
    ...
    for col in valid_traces.columns:
        ...
        source2 = ColumnDataSource(data=dict(peak_pos=peak_df['Critical Point pos'],
                                             peak_heights=peak_df['Absolute Critical Point heights']))  # Peaks
        source3 = ColumnDataSource(data=dict(trough_pos=trough_df['Critical Point pos'],
                                             trough_heights=trough_df['Absolute Critical Point heights']))  # Troughs
